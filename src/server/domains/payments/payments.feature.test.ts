...
describe.skipIf(!hasTestDb)("online payments (wallet and card)", () => {
  ...
  describe("waitlist auto-join (§6)", () => {
    async function fullEvent() {
      const setup = await makeOrganizerAndEvent();
      await db.event.update({ where: { id: setup.event.id }, data: { maxPlayers: 1 } });
      const holder = await makePlayer("Hal", "+353830000010");
      await holder.caller.rsvps.create({ eventId: setup.event.id, paymentMethod: "CASH" });
      return { ...setup, holder };
    }

    it("moves the first auto-join entry in when a spot opens, ahead of earlier notify-me entries", async () => {
      const { event, holder } = await fullEvent();
      const notifyMe = await makePlayer("Nina", "+353830000011");
      const auto = await makePlayer("Aoife", "+353830000012");
      await db.pushSubscription.create({ data: { userId: auto.user.id, endpoint: "https://push.example/aoife", p256dh: "k", auth: "a" } });
      await fundWallet(auto.caller, 2000);
      await notifyMe.caller.waitlist.join({ eventId: event.id });
      await auto.caller.waitlist.join({ eventId: event.id, preference: { mode: "AUTO", paymentMethod: "WALLET" } });
      expect((await auto.caller.events.getBySlug({ slug: event.slug })).viewerWaitlistPosition).toBe(1); // ahead, despite joining later
      fakePushSender.reset();

      await holder.caller.rsvps.drop({ eventId: event.id });

      const moved = await db.rsvp.findFirstOrThrow({ where: { eventId: event.id, userId: auto.user.id } });
      expect(moved).toMatchObject({ status: "GOING", paymentMethod: "WALLET", paymentStatus: "HELD" });
      expect(await db.waitlistEntry.findMany({ where: { eventId: event.id } })).toMatchObject([{ userId: notifyMe.user.id }]);
      expect(fakePushSender.sent.map((entry) => entry.message.title)).toEqual(["You're in!"]); // the spot is gone: notify-me hears nothing
    });

    it("moves a card entry in and charges it at once on an already-confirmed event", async () => {
      const { organizerCaller, event, holder } = await fullEvent();
      await organizerCaller.events.confirm({ eventId: event.id });
      const auto = await makePlayer("Aoife", "+353830000012");
      const { setupIntentId } = await auto.caller.rsvps.beginCardSetup({ eventId: event.id });
      fakePaymentGateway.completeSetupIntent(setupIntentId, "visa");
      await auto.caller.waitlist.join({ eventId: event.id, preference: { mode: "AUTO", paymentMethod: "CARD", setupIntentId } });
      expect(fakePaymentGateway.chargesMade).toHaveLength(0); // joining the waitlist charges nothing

      await holder.caller.rsvps.drop({ eventId: event.id });

      expect(fakePaymentGateway.chargesMade.map((c) => c.amountCents)).toEqual([850]);
      expect(await db.rsvp.findFirstOrThrow({ where: { userId: auto.user.id } })).toMatchObject({ paymentStatus: "CHARGED" });
    });

    it("skips a wallet entry that can't cover the price, tells them, and moves the next one in", async () => {
      const { event, holder } = await fullEvent();
      const broke = await makePlayer("Bo", "+353830000013");
      const rich = await makePlayer("Rae", "+353830000014");
      await db.pushSubscription.create({ data: { userId: broke.user.id, endpoint: "https://push.example/bo", p256dh: "k", auth: "a" } });
      await fundWallet(rich.caller, 2000);
      await broke.caller.waitlist.join({ eventId: event.id, preference: { mode: "AUTO", paymentMethod: "WALLET" } });
      await rich.caller.waitlist.join({ eventId: event.id, preference: { mode: "AUTO", paymentMethod: "WALLET" } });
      fakePushSender.reset();

      await holder.caller.rsvps.drop({ eventId: event.id });

      expect((await db.rsvp.findFirstOrThrow({ where: { userId: rich.user.id } })).status).toBe("GOING");
      expect(await db.rsvp.count({ where: { userId: broke.user.id } })).toBe(0);
      expect(await db.waitlistEntry.count({ where: { userId: broke.user.id } })).toBe(1); // keeps their place
      expect(fakePushSender.sent.some((entry) => entry.message.title === "We couldn't move you in")).toBe(true);
    });

    it("notifies notify-me entries only when no auto-join entry took the spot", async () => {
      const { event, holder } = await fullEvent();
      const notifyMe = await makePlayer("Nina", "+353830000011");
      await db.pushSubscription.create({ data: { userId: notifyMe.user.id, endpoint: "https://push.example/nina", p256dh: "k", auth: "a" } });
      await notifyMe.caller.waitlist.join({ eventId: event.id });
      fakePushSender.reset();

      await holder.caller.rsvps.drop({ eventId: event.id });

      expect(fakePushSender.sent.map((entry) => entry.message.title)).toEqual(["A spot opened up"]);
    });

    it("switches mode keeping the join time, and needs online payments for auto-join", async () => {
      const { event, group, organizerCaller } = await fullEvent();
      const player = await makePlayer("Nina", "+353830000011");
      const entry = await player.caller.waitlist.join({ eventId: event.id });

      const switched = await player.caller.waitlist.setMode({ eventId: event.id, preference: { mode: "AUTO", paymentMethod: "WALLET" } });
      expect(switched).toMatchObject({ promotionMode: "AUTO", paymentMethod: "WALLET" });
      expect(switched.createdAt).toEqual(entry.createdAt);

      const back = await player.caller.waitlist.setMode({ eventId: event.id, preference: { mode: "MANUAL" } });
      expect(back).toMatchObject({ promotionMode: "MANUAL", paymentMethod: null });

      const startsAt = new Date(Date.now() + 86_400_000);
      const cashOnly = await organizerCaller.events.create({
        groupId: group.id, title: "Cash", startsAt, endsAt: new Date(startsAt.getTime() + 3_600_000), location: "A",
        cutoffAt: new Date(Date.now() + 3_600_000), totalCostCents: 500, maxPlayers: 1, pricingMode: PricingMode.FIXED_PER_HEAD, cashAllowed: true,
      });
      await expect(
        player.caller.waitlist.join({ eventId: cashOnly.id, preference: { mode: "AUTO", paymentMethod: "WALLET" } }),
      ).rejects.toThrow("Auto-join needs online payments");
      await expect(player.caller.waitlist.join({ eventId: event.id, preference: { mode: "AUTO" } })).rejects.toThrow("Choose wallet or card");
    });

    it("moves auto-join entries into room the organizer added (worker sweep)", async () => {
      const { organizerCaller, event } = await fullEvent();
      const auto = await makePlayer("Aoife", "+353830000012");
      await fundWallet(auto.caller, 2000);
      await auto.caller.waitlist.join({ eventId: event.id, preference: { mode: "AUTO", paymentMethod: "WALLET" } });
      ...
      await organizerCaller.events.edit({
        ...
        cutoffAt: current.cutoffAt, totalCostCents: current.totalCostCents, pricingMode: current.pricingMode,
        cashAllowed: true, onlineAllowed: true, maxPlayers: 2,
      });

      const { promoteWaitlists } = await import("@/server/domains/rsvps");
      await promoteWaitlists(db, fakePaymentGateway, new Date());

      expect((await db.rsvp.findFirstOrThrow({ where: { userId: auto.user.id } })).status).toBe("GOING");
    });
  });
});
